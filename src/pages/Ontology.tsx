...
const CHANNELS: ChannelDef[] = [
  {
    ...
    label: "iMessage",
    icon: MessageSquare,
    color: "text-vanta-accent",
    colorBg: "bg-vanta-accent-faint",
    colorBorder: "border-vanta-accent-border",
    source: "linq",
  ...
  },
  {
    ...
    label: "Native Phone",
    icon: Phone,
    color: "text-vanta-accent-phone",
    colorBg: "bg-vanta-accent-phone-faint",
    colorBorder: "border-vanta-accent-phone-border",
    source: "phone",
    ...
    narrative: "The phone call is the highest-density, most systematically ignored communication channel. Vanta doesn't ask the user to change behavior — it owns the infrastructure. Every call transiting Vanta's network becomes an intelligence event with speaker attribution, commitment extraction, and relationship signal detection.",
  },
  {
    id: "zoom",
    label: "Zoom Meetings",
    icon: Video,
    color: "text-vanta-accent-zoom",
    colorBg: "bg-vanta-accent-zoom-faint",
    colorBorder: "border-vanta-accent-zoom-border",
    source: "recall",
    ...
    signalTypes: ["MEETING"],
    narrative: "Meetings are where decisions crystallize and commitments are made in front of witnesses. The Recall.ai integration captures the full transcript, identifies speakers, and runs the same two-stage AI pipeline — but the meeting context adds structure that free-form text lacks. Attendee matching against the signal history creates pre-meeting briefs automatically.",
  },
  {
    id: "email",
    label: "Email",
    icon: Mail,
    color: "text-vanta-accent-teal",
    colorBg: "bg-vanta-accent-teal-faint",
    colorBorder: "border-vanta-accent-teal-border",
    source: "gmail",
    ...
    narrative: "Email is the workhorse — lower signal density than iMessage, but higher formality. Investment memos, partnership proposals, and formal introductions flow through email. The Gmail integration captures thread context and metadata (CC lists, forwarding chains) that reveal network topology invisible in other channels.",
  },
  {
    id: "calendar",
    label: "Calendar",
    icon: Calendar,
    color: "text-vanta-accent-amber",
    colorBg: "bg-vanta-accent-amber-faint",
    colorBorder: "border-vanta-accent-amber-border",
    source: "calendar",
    ...
    signalTypes: ["MEETING"],
    narrative: "The calendar is not a signal source — it's a signal trigger. When a meeting appears on the calendar, Vanta cross-references every attendee against the full signal history and generates a contextual brief. You walk into every meeting already knowing what was said, promised, and left unresolved.",
  },
];
    ...
    <div className="min-h-screen bg-background">
      ...
      <nav className="sticky top-0 z-50 flex items-center justify-between px-5 py-5 md:px-10 bg-background/95 backdrop-blur-md border-b border-vanta-border">
        ...
        <div className="flex items-center gap-3">
          <a href="/phone-fmc?skip-auth=1" className="font-mono text-[10px] text-vanta-text-low uppercase tracking-wider hover:text-vanta-accent transition-colors">Phone FMC</a>
          <a href="/signals?skip-auth=1" className="font-mono text-[10px] text-vanta-text-low uppercase tracking-wider hover:text-vanta-accent transition-colors">Signals</a>
          <a href="/graph?skip-auth=1" className="font-mono text-[10px] text-vanta-text-low uppercase tracking-wider hover:text-vanta-accent transition-colors">Graph</a>
        </div>
      </nav>

      <article className="max-w-[800px] mx-auto px-5 py-12 md:px-10 md:py-20">
        {/* Header */}
        ...
        <p className="font-sans text-[14px] text-vanta-text-mid leading-relaxed mb-12 max-w-xl">
          Vanta treats every communication channel as a signal source — not a feature to integrate, but infrastructure to own. The ontology below maps how raw communication becomes classified intelligence across five channels.
        </p>
        ...
        <section className="mb-14">
          <div className="border-l-2 border-primary pl-5 mb-8">
            ...
            <p className="font-sans text-[13px] text-vanta-text-mid leading-relaxed">
              Conventional products sit on top of channels and ask the user to change behavior. Vanta sits underneath them. When you own the iMessage relay, the SIM card, the meeting bot, and the email connection, capture becomes invisible. The user's only job is to keep doing what they already do. Vanta's job is to make sure nothing is lost.
            </p>
          </div>
        </section>
        ...
        <section className="mb-14">
          <p className="font-mono text-[9px] uppercase tracking-[0.2em] text-vanta-text-low mb-6">
            Channel Architecture
          </p>
            ...
            {CHANNELS.map((ch) => (
              <div key={ch.id} className={`border ${ch.colorBorder} overflow-hidden`}>
                ...
                <div className={`flex items-center gap-3 px-5 py-3 ${ch.colorBg}`}>
                  ...
                  <span className={`font-mono text-[12px] uppercase tracking-[0.14em] font-bold ${ch.color}`}>
                    {ch.label}
                  </span>
                  <span className="font-mono text-[9px] text-vanta-text-muted uppercase tracking-wider ml-auto">
                    source: {ch.source}
                  </span>
                </div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {ch.signalTypes.map((st) => {
                        ...
                        return (
                          <span
                            key={st}
                            className={`inline-block px-2 py-0.5 font-mono text-[9px] uppercase tracking-[0.12em] border ${stDef?.color || "text-vanta-text-muted"} ${stDef?.bg || "bg-card"} ${stDef?.border || "border-vanta-border"}`}
                          >
                            {st}
                          </span>
                        );
        ...
        <section className="mb-14">
          ...
          <p className="font-sans text-[13px] text-vanta-text-mid leading-relaxed mb-6 italic">
            Every signal — regardless of channel — passes through the same four-stage pipeline. The channel determines capture method; the pipeline determines intelligence quality.
          </p>
          ...
          <div className="border border-vanta-border">
            {PIPELINE_STAGES.map((stage, i) => (
              <div key={stage.id}>
                ...
                <div className={`flex items-start gap-4 p-4 ${i === 0 ? "bg-vanta-accent-faint" : i === PIPELINE_STAGES.length - 1 ? "bg-vanta-accent-faint" : "bg-card"} ${i > 0 ? "border-t border-vanta-border-mid" : ""}`}>
        ...
        <section className="mb-12">
          <div className="border border-vanta-accent-border bg-vanta-accent-faint p-6">
            <p className="font-sans text-[14px] text-foreground font-bold mb-2">
              The Architecture Is the Product
            </p>
            <p className="font-sans text-[13px] text-vanta-text-mid leading-relaxed mb-3">
              Most intelligence products are features bolted onto someone else's infrastructure. Vanta's advantage is that the capture layer, the classification layer, and the delivery layer are all first-party. No vendor dependency. No API rate limits. No data residency questions.
            </p>
            <p className="font-sans text-[13px] text-vanta-text-mid leading-relaxed">
        ...
        <div className="flex flex-wrap gap-2">
          {["Ontology", "Signal Architecture", "iMessage", "Phone", "Zoom", "Email", "Calendar", "AI Pipeline", "FMC"].map((tag) => (
            <span
              key={tag}
              className="font-mono text-[9px] uppercase tracking-[0.15em] text-vanta-text-muted border border-vanta-border px-2 py-1"
            >
              {tag}
            </span>
          ))}
