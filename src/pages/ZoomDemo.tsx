...
function generateSessionPDF() {
  ...
  const addLine = (size: number, weight: string, text: string, color = "#111111") => {
  ...
  // Header
  addLine(22, "bold", "VANTA SIGNAL");
  addLine(9, "normal", "SESSION SUMMARY", "#888888");
  y += 4;

  // Metadata strip
  doc.setDrawColor("#E0E0DE");
  doc.line(margin, y, pw - margin, y);
  y += 6;
  addLine(8, "normal", `Duration: 11m 08s  |  Participants: 3  |  Signals: 3  |  ${new Date().toLocaleDateString()}`, "#555555");
  y += 4;

  // Narrative
  addLine(11, "bold", "Meeting narrative");
  addLine(9, "normal", "The meeting converged on a $12M pre-money Series A structure, confirmed investor appetite, and reinforced the vertical SaaS thesis.");
  y += 4;

  // Key takeaways
  addLine(11, "bold", "Key takeaways");
  SUMMARY_POINTS.forEach((point) => {
    addLine(9, "normal", `•  ${point}`);
  });
  y += 4;

  // Detected signals
  addLine(11, "bold", "Detected signals");
  DEMO_SIGNALS.forEach((s) => {
    addLine(9, "bold", `[${s.type}] ${s.ts} — ${s.speaker}`);
    addLine(9, "normal", `"${s.text}"`);
    y += 1;
  });
  y += 4;

  // Action items
  addLine(11, "bold", "Action items");
  FOLLOW_UPS.forEach((item) => {
    addLine(9, "normal", `→  ${item}`);
  });
  y += 8;

  // Participants
  addLine(11, "bold", "Participants");
  addLine(9, "normal", "You (William Traylor)");
  BASE_PARTICIPANTS.forEach((p) => {
    addLine(9, "normal", `${p.name} — ${p.email}`);
  });
  ...
  doc.line(margin, y, pw - margin, y);
  y += 5;
  addLine(7, "normal", "Generated by Vanta Signal · vantasignal.lovable.app", "#AAAAAA");

  doc.save("vanta-session-summary.pdf");
...
function AudioWaveform() {
  return (
    <div className="flex h-8 items-end gap-[3px]">
      {[0.4, 0.7, 0.5, 1, 0.6, 0.8, 0.3, 0.9, 0.5, 0.7, 0.4, 0.6, 0.8, 0.5, 0.3, 0.7, 0.9, 0.4, 0.6, 0.5].map((h, i) => (
        <div
          key={i}
          className="w-[3px] bg-foreground/60 animate-pulse"
          style={{
            height: `${h * 100}%`,
...
export default function ZoomDemo() {
  ...
  const isStreamingOrLater = ["streaming", "detecting", "complete"].includes(phase);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8 px-4 py-8">
      ...
      <section className={`space-y-3 border border-border bg-card p-4 transition-opacity ${!["invited", "streaming", "detecting", "complete"].includes(phase) ? "pointer-events-none opacity-40" : "opacity-100"}`}>
        <div className="flex items-center gap-2">
          <div className={`flex h-6 w-6 items-center justify-center border text-[10px] font-mono font-bold ${isStreamingOrLater ? "border-foreground bg-foreground text-background" : "border-border text-muted-foreground"}`}>
            {isStreamingOrLater ? <CheckCircle2 className="h-3.5 w-3.5" /> : "3"}
          </div>
        ...
        {(streamStatus === "streaming" || phase === "detecting" || isComplete) && (
            ...
            <div className="flex items-center justify-between">
              ...
              {streamStatus === "streaming" && (
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-mono text-[8px] uppercase tracking-wider text-red-500">Rec</span>
                </div>
              )}
            </div>
        ...
        {/* Audio waveform during streaming */}
        {streamStatus === "streaming" && (
          <div className="space-y-2 border border-border bg-muted/40 p-3">
            <div className="flex items-center gap-2">
              <Headphones className="h-3 w-3 text-muted-foreground" />
              <p className="font-mono text-[8px] uppercase tracking-wider text-muted-foreground">Audio capture — RTMS raw stream</p>
            </div>
            <AudioWaveform />
          </div>
        )}
      ...
      <section className={`space-y-3 border border-border bg-card p-4 transition-opacity ${!["detecting", "complete"].includes(phase) ? "pointer-events-none opacity-40" : "opacity-100"}`}>
        <div className="flex items-center gap-2">
          <div className={`flex h-6 w-6 items-center justify-center border text-[10px] font-mono font-bold ${isComplete ? "border-foreground bg-foreground text-background" : "border-border text-muted-foreground"}`}>
            {isComplete ? <CheckCircle2 className="h-3.5 w-3.5" /> : "4"}
          </div>
        ...
        <p className="font-mono text-[10px] leading-relaxed text-muted-foreground">
          Vanta AI classifies the live transcript in real time, surfacing decisions, investment signals, and strategic insights as they happen.
        </p>
      ...
      {/* Post-session: Audio + Video capture blocks */}
      {isComplete && (
        <section className="space-y-4 border border-border bg-card p-4">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="h-4 w-4 text-foreground" />
            <h2 className="font-mono text-xs uppercase tracking-wider text-foreground">Media capture</h2>
          </div>
          ...
          <div className="space-y-3 border border-border p-3">
            <div className="flex items-center gap-2">
              ...
              <p className="font-mono text-[9px] uppercase tracking-wider text-muted-foreground">Audio recording</p>
            </div>
            <div className="flex items-center gap-3">
              <button
                onClick={() => toast("Audio playback requires native SDK")}
                className="flex h-8 w-8 shrink-0 items-center justify-center border border-border text-foreground transition-colors hover:bg-muted"
              ...
              <div className="flex-1 space-y-1">
                <div className="h-1.5 w-full bg-muted">
                  <div className="h-full w-3/4 bg-foreground/30" />
                </div>
                <div className="flex justify-between">
                  <span className="font-mono text-[8px] text-muted-foreground">Raw audio captured via RTMS</span>
                  <span className="font-mono text-[8px] text-muted-foreground">11:08</span>
                </div>
              </div>
            </div>
            <p className="font-mono text-[8px] text-muted-foreground">Format: PCM 48kHz · Size: 12.4 MB</p>
          </div>
          ...
          <div className="space-y-3 border border-border p-3">
            <div className="flex items-center gap-2">
              ...
              <p className="font-mono text-[9px] uppercase tracking-wider text-muted-foreground">Video recording</p>
              <span className="ml-auto font-mono text-[8px] text-muted-foreground">47.2 MB</span>
            </div>
            <div className="grid grid-cols-3 gap-2">
              {["You", "Sarah Chen", "Marcus Rivera"].map((name) => (
                <div key={name} className="flex flex-col items-center gap-1 border border-border bg-muted/40 p-2">
                  <div className="flex h-6 w-6 items-center justify-center border border-border font-mono text-[8px] font-bold text-muted-foreground">
                    {name.split(" ").map((p) => p[0]).join("").slice(0, 2)}
                  </div>
                  <span className="font-mono text-[7px] uppercase tracking-wider text-muted-foreground">{name}</span>
                </div>
              ))}
            </div>
            <p className="font-mono text-[8px] text-muted-foreground">Recording saved · H.264 720p · 3 streams</p>
            <button
              onClick={() => toast("Video export requires native SDK")}
              className="inline-flex items-center gap-1.5 border border-border px-3 py-1.5 font-mono text-[9px] uppercase tracking-wider text-foreground transition-colors hover:bg-muted"
            >
      ...
      {isComplete && <SummaryPanel onReset={resetDemo} />}
    </div>
  );
}
