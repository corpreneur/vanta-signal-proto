...
import { ArrowLeft, Video, Users, MessageSquare, Phone, Mail, StickyNote, TrendingUp, Download, Send, ExternalLink, Copy } from "lucide-react";
...
export default function Briefing() {
  ...
  const handleEmailAttendees = async () => {
    ...
    try {
      ...
      // Build a rich plain-text body with per-attendee talking points
      const talkingPointsByAttendee = names.map((name) => {
        const matched = brief.matched_signals.filter((ms) => ms.matched_attendee === name);
        const points = matched.map((ms) => `  • [${ms.signal_type}] ${ms.summary}`).join("\n");
        return `${name}:\n${points || "  (No specific talking points)"}`;
      }).join("\n\n");

      const bodyText = [
        `PRE-MEETING BRIEF: ${meeting?.title || "Upcoming Meeting"}`,
        meetingTime ? `Date: ${meetingTime}` : "",
        "",
        brief.brief_text,
        "",
        "KEY TALKING POINTS BY ATTENDEE:",
        talkingPointsByAttendee,
        "",
        "---",
